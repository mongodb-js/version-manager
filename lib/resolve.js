...
module.exports = function(opts, fn){
  ...
  opts.buildvariant = opts.buildvariant || null;
...
function mci(opts, fn){
  if(!opts.buildvariant){
    return fn(new Error('Missing required paramater `buildvariant`'));
  }
  ...
  request.get({url: url, json: true}, function(err, res, body){
    ...
    var dl = 'https://s3.amazonaws.com/mciuploads/mongodb-mongo-' + opts.branch +
      '/'+opts.buildvariant +'/'+opts.version+'/binaries';

    var s = 'mongodb_mongo_' + opts.branch + '_' + opts.buildvariant,
      basename = body.builds.filter(function(b){
        return b.indexOf(s) > -1;
      })[0];
    // @todo: test across all buildvariants as I believe this may be different for some.
